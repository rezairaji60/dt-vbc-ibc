...
def plot_dt_vbc_components(path, title_prefix, vars_, degree, coeffs, prefixes, f_exprs, bounds, X0_box, Xu_box, seed=0):
    xs, ys, pts = make_grid(bounds, 220)
    ...
    for i, ax in enumerate(axes):
        ...
        x0, x1, y0, y1 = X0_box
        u0, u1, v0, v1 = Xu_box
        ax.add_patch(plt.Rectangle((x0, y0), x1 - x0, y1 - y0, fill=False, edgecolor='tab:blue', linewidth=2.0))
        ax.add_patch(plt.Rectangle((u0, v0), u1 - u0, v1 - v0, fill=False, edgecolor='tab:orange', linewidth=2.0))
        for tr in trajs:
