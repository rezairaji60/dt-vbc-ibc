...
On `X`, the box factor `1-x^2` is nonnegative; under the antecedent `b(x)<=0`, `-x` is nonnegative; every multiplier displayed above is a square or a positive multiple of a square. Thus the implication is certified algebraically at degree one. The repository also stores the certificate in a separate implication-proof schema and reconstructs the box generators, antecedent generator, exact PSD Gram blocks, and residual in rational arithmetic. This is fixed-certificate verification; it is not a claim that joint frame/multiplier synthesis is convex.
