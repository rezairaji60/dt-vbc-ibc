...
We address both questions. First, we give an exact, bidirectional identification of globally scaled IBCs with a precisely anchored path-matrix VBC subclass. The identification includes the polynomial residuals used in synthesis, making it possible to transport SOS witnesses. Second, we prove a genuine fixed-degree separation. A finite-order transition forces any sign-changing scalar comparison certificate to have unit gain and to be invariant. When the transition is linear and has no eigenvalue one, an invariant affine function is constant. This obstructs the terminal frame of every affine scaled IBC, independently of the frame count. Cyclic affine VBCs escape the obstruction by permuting their components rather than requiring a single terminal function to be invariant.

Graph-structured certificates are not new in themselves. In particular, path-complete barrier functions use graphs to express safety conditions for switched systems, and their completeness and ordering have been investigated by Anand et al.~\cite{anand}. Our result concerns an autonomous transition, anchored separation, and the globally scaled IBC chain with a terminal scalar loop. It is not a general ordering theorem for path-complete graphs. Likewise, numerical-to-rational SOS verification has established precedents~\cite{peyrl}; our contribution is its integration with exact certificate transport and a reproducible diagnosis of these specific representations.
...
\begin{theorem}[No affine global scaled IBC]\label{thm:obstruction}
Let $f(x)=Rx$, where $R^r=I$ and $1$ is not an eigenvalue of $R$. Let $X$ have nonempty interior and satisfy $R(X)=X$. For any nonempty $X_0,X_u\subset X$, no robust global scaled forward or backward IBC consisting entirely of affine frames exists, for any finite frame count.
\end{theorem}
\begin{proof}
For a forward IBC, start at $x_0\in X_0$. The chain inequalities imply
\[
 b_m(R^{m-1}x_0)\le
 -\delta_0\Big/\prod_{i=1}^{m-1}\lambda_i<0.
\]
The terminal frame is positive on $X_u$, so it takes both signs. Its terminal inequality and Lemma~\ref{lem:finite} imply $b_m\circ R=b_m$.

For a backward IBC, choose $u\in X_u$. Since $R$ is bijective on $X$, iterating the chain at $R^{-1}u,\ldots,R^{-(m-1)}u$ gives
\[
 b_m(R^{-(m-1)}u)\le
 -\delta_u\prod_{i=1}^{m-1}\lambda_i<0.
\]
The same frame is positive on $X_0$. Its terminal inequality again implies invariance.

In either case write $b_m(x)=c^\top x+d$. Invariance on a set with nonempty interior implies $c^\top(R-I)=0$. The assumption on the spectrum gives $c=0$. Thus $b_m$ is constant, contradicting its two signs. Empty products cover $m=1$.
\end{proof}
...
\]
An anchored forward VBC with $2n$ affine components exists. No global scaled IBC with affine frames exists, regardless of frame count. A quadratic scalar certificate, hence repeated quadratic IBC frames in both directions, does exist.
\end{corollary}
\begin{proof}
Take
...
\end{tikzpicture}
\caption{Exact two-dimensional structural example. The four initial-box images form a periodic orbit of sets. Cyclic comparison preserves four affine facets, whereas a terminal affine frame would have to be constant. Arrows illustrate the discrete rotation, not continuous trajectories.}\label{fig:rotation}
\end{figure}

This is a separation of globally scaled template classes, not a claim against unrestricted implication IBCs or arbitrary scalar functions. The four-dimensional example is a block extension testing implementation generality, not a claim of high-dimensional computational scalability. The example is deliberately constructed; it is not represented as an external benchmark.
...
BarrierBench is a recent collection covering both controlled and autonomous, continuous- and discrete-time problems~\cite{barrierbench,data}. It is a useful provenance source, not evidence that every entry is challenging or that the collection is already a long-established community standard. We reproduce the autonomous entry whose transition simplifies exactly to
\[
 f(x)=(-x_2/100,\ x_1/100),
\]
with $X_0=[0.1,0.4]\times[0.1,0.55]$ and $X_u=[0.45,0.5]\times[0.6,1]$. We add the invariant verification box $[-1.2,1.2]^2$, without changing the transition or either safety set. It is a contraction and should not be used to claim a VBC advantage. Its initial states leave the initial box after one step.
...
The strongest negative result in the paper is consequently analytical: the rotation systems have no affine global scaled IBC for any finite frame count. Their positive cyclic witnesses have exact zero residual and are independently replayed. The four-dimensional instance extends the same construction rather than introducing a new physical application. We make no broad scalability claim from these dimensions and no performance ranking against external tools with different semantics, controllers, or acceptance policies.

\section{Conclusion}
Global scaled IBCs have an exact anchored path-matrix interpretation, provided direction, reciprocal weights, strict separation, and propagation domains are aligned. This interpretation transports both certificates and their SOS proofs. General matrix support is not itself evidence of expressive power; a finite-order obstruction instead proves a strict affine--quadratic separation between cyclic VBCs and globally scaled IBCs of arbitrary finite length. Periodicity also shows why a strictly positive propagation reserve can create artificial infeasibility. A Julia implementation separates numerical synthesis from exact-rational replay and makes normalization, domain repair, provenance, and scalar baselines explicit. Future comparisons should retain this distinction between representation, template nonexistence, relaxation failure, and numerical uncertainty.
